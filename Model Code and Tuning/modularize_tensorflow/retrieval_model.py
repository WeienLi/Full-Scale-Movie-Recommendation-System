...
import wandb
def get_unique_value(unique_movie_titles1,unique_years_binned1,unique_lengths_binned1,unique_user_ids1,
                     unique_user_genders1,unique_user_ages_binned1,unique_languages1,unique_occupations1):
...
def compute_unique(N_movies,N):
    ...
    """
    global unique_movie_titles,unique_years_binned,unique_lengths_binned,unique_user_ids,unique_user_genders,\
           unique_user_ages_binned,unique_languages,unique_occupations
...
class RetrievalMovieModel(tf.keras.Model):
    ...
    def call(self, inputs):
        ...
        feature_embedding = tf.concat(values=[self.title_embedding(inputs['candidate_title']),
...
def creating_index(k,model_1):
    ...
    index.index_from_dataset(
        tf.data.Dataset.zip(
            (movies.batch(200).map(lambda x: x["candidate_title"]), movies.batch(200).map(model_1.candidate_model)))
    )
    return index